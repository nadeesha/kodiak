...
services.factory('userService', ['$rootScope', '$localStorage', '$http', 'GRIZZLY_URL',
    function($rootScope, $localStorage, $http, GRIZZLY_URL) {
        ...
        var setUserType = function() {
            if (model.restored && model.affiliation && moment().isBefore(model.expiration))
                $rootScope.user.type = 'ORG';
            else if (model.restored && !model.affiliation && moment().isBefore(model.expiration))
                $rootScope.user.type = 'PERSONAL';
            else
                $rootScope.user.type = 'NEW';
        }
